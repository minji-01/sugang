...
df = pd.DataFrame(data_2nd, columns=["구분", "교과(군)", "과목 유형", "과목", "학점", "1학기", "전공"])

st.title("2학년 선택과목 수강신청")
st.write("✅ 전공 과목 표시")

# 체크박스를 표 형태로
for i, row in df.iterrows():
    cols = st.columns([1,1,1,3,1,1,1])
    cols[0].write(row["구분"])
    cols[1].write(row["교과(군)"])
    cols[2].write(row["과목 유형"])
    cols[3].write(row["과목"])
    cols[4].write(row["학점"])
    cols[5].write("🔒")  # 1학기 개설 X
    # 체크박스: key를 고유값으로 지정
    checked = cols[6].checkbox("", key=f"course_{i}")
